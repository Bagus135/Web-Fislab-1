...
export const editScore = async (req : Request, res : Response) =>{
    try {
        ...
        if(!noJudul|| !PreLab|| !InLab|| !Abstrak|| !Pendahuluan||!Metodologi||!Pembahasan||!Kesimpulan||!Format||!nilaiTotal||!comment) return res.status(422).json({error: `Fill All Fields`})
